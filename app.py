...
if not all(col in df.columns for col in required_cols):
    st.warning(f"⚠️ Kolom belum lengkap. Harus ada: {required_cols}")
else:
    # ========== 1. Tambah ==========
    ...
    if not df.empty:
        ...
        estimasi_new = st.date_input("Edit Estimasi", pd.to_datetime(selected["estimasi_selesai"]), key="edit_estimasi")
    ...
    st.download_button("⬇️ Excel", df.to_csv(index=False).encode(), file_name="jadwal_konsultasi.csv")
    ...
    if not reminder.empty:
        ...
        pesan = f"🔔 Reminder pekerjaan selesai H-1 ({besok}):\\n"
